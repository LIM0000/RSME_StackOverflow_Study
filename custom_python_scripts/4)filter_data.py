...
# Filter answers data with PostTypeId="2" (we take all answers, does not matter how many user has answered)
# Output -> result_2.2.csv
...
try:
    ...
    with open("D:\\Download (From Chrome)\\result_2.2.csv", 'a', newline='') as f:
        writer = csv.writer(f)
        ...
        for i, (event, element) in enumerate(context):
            ...
            try:
                # Check is PostTypeId = 1
                if element.attrib["PostTypeId"] == "2" and element.attrib["OwnerUserId"] in user_id_set:
